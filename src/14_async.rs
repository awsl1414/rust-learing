...
async fn main() {
    ...
    // 正常完成的任务
    match tokio::time::timeout(Duration::from_millis(1500), async_task("正常任务", 1000)).await
    {
    ...
    // 超时的任务
    match tokio::time::timeout(Duration::from_millis(500), async_task("慢速任务", 1000)).await {
        Ok(result) => println!("任务成功完成: {}", result),
    ...
    // 创建数字流
    let number_stream = stream::iter(1..=5).then(|n| async move {
    ...
    for (i, url) in urls.into_iter().enumerate() {
        let handle = tokio::spawn(async move { simulate_http_request(url, 200 + i * 100).await });
        handles.push(handle);
    }
    ...
    for i in 1..=5 {
        ...
        let handle = tokio::spawn(async move { use_connection_from_pool(pool_clone, i).await });
        handles.push(handle);
    }
...
impl ConnectionPool {
    fn new(size: usize) -> Self {
        let connections = (1..=size).map(|id| Connection { id }).collect();
